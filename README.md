...
- **SRUM fresh-attempt result (2026-03-27)**: outcome remains metadata-probe-only. The repository currently lacks real ESE catalog/record decoding required for truthful SRUM row/cell extraction, so no SRUM row-level provider/CLI/GUI/timeline surfaces were added in this pass.
- **SRUM design-readiness note (current pass)**: next credible SRUM implementation must begin with one tiny real ESE-backed subset (recommended first candidate: `network_data_usage` table), and only after catalog+record+cell decoding is implemented and tested; synthetic marker/blob formats remain explicitly forbidden.
