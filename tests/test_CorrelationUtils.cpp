...
int runCorrelationUtilsTests() {
  ...
  if (pathCorrelationTypeLabel(ancestor.type) != "Parent path (artifact ancestor)") return 1;
  ...
  if (pathCorrelationTypeLabel(descendant.type) != "Child path (artifact descendant)") return 1;
  ...
  if (pathCorrelationTypeLabel(unrelated.type) != "No direct path correlation") return 1;
